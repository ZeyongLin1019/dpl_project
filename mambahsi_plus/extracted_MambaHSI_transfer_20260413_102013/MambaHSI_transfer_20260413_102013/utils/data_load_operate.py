...
def load_data(data_set_name, data_path='./data', cube_npy_path='', gt_npy_path=''):
    if cube_npy_path or gt_npy_path:
        if not cube_npy_path or not gt_npy_path:
            raise ValueError("Both cube_npy_path and gt_npy_path must be provided for custom .npy data.")
        data = np.load(cube_npy_path)
        labels = np.load(gt_npy_path)
        return data, labels
